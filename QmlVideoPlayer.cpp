...
{
    if (_media) return QUrl(_media->currentLocation());
    else return QUrl();
}
...
void VlcQmlVideoPlayer::setUrl(const QUrl &url)
{
    ...
    if(url.isLocalFile()) _media = new VlcMedia(url.toLocalFile(), true, _instance);
    else _media = new VlcMedia(url.toString(QUrl::FullyEncoded), false, _instance);    
